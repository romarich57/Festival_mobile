...
import { buildPasswordResetEmailUrl, buildVerificationEmailUrl, } from '../../services/email.js';
test('buildVerificationEmailUrl should target the backend verification endpoint', () => {
    ...
    assert.ok(verificationUrl.startsWith(`${PUBLIC_BACKEND_URL.replace(/\/$/, '')}/api/auth/verify-email?token=`));
...
test('buildPasswordResetEmailUrl should target the backend password reset endpoint', () => {
    ...
    assert.ok(resetUrl.startsWith(`${PUBLIC_BACKEND_URL.replace(/\/$/, '')}/api/auth/reset-password?token=`));
    assert.strictEqual(parsedUrl.searchParams.get('token'), token);
});
