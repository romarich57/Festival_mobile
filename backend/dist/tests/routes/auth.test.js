...
async function storeVerificationTokenForUser(userId, token, expiresAt) {
...
async function storePasswordResetTokenForUser(userId, token, expiresAt) {
    const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
    await pool.query(`
        UPDATE users
...
async function createRefreshTokenRecord(userId, expiresAt = new Date(Date.now() + 60_000)) {
...
test('POST /register - should create new user with valid data', async () => {
    ...
    await getRouteHandler('/register', 'post')?.(mockReq, mockRes, () => { });
...
test('POST /login - should login with valid credentials', async () => {
    ...
    const mockRes = createMockResponse();
    await getRouteHandler('/login', 'post')?.(mockReq, mockRes, () => { });
...
test('POST /logout - should clear auth cookies with the active cookie policy', async () => {
    ...
    try {
        ...
        const mockRes = createMockResponse();
        await getRouteHandler('/logout', 'post')?.(mockReq, mockRes, () => { });
...
test('GET /verify-email - should redirect to the mobile deep link on success', async () => {
    ...
    const mockRes = createMockResponse();
    await getRouteHandler('/verify-email', 'get')?.(mockReq, mockRes, () => { });
...
test('POST /password/forgot - should return the generic success message for an unknown email', async () => {
    ...
    const mockRes = createMockResponse();
    await getRouteHandler('/password/forgot', 'post')?.(mockReq, mockRes, () => { });
    assert.strictEqual(mockRes.statusCode, 200);
    assert.strictEqual(mockRes.jsonData?.message, 'Si un compte existe pour cet email, un lien de réinitialisation a été envoyé.');
...
test('POST /password/forgot - should store a reset token and send an email for a known account', async () => {
    ...
    await getRouteHandler('/password/forgot', 'post')?.(mockReq, mockRes, () => { });
    assert.strictEqual(mockRes.statusCode, 200);
    assert.strictEqual(mockRes.jsonData?.message, 'Si un compte existe pour cet email, un lien de réinitialisation vient d’être envoyé.');
...
test('GET /reset-password - should redirect to the mobile deep link', async () => {
    ...
    const mockRes = createMockResponse();
    await getRouteHandler('/reset-password', 'get')?.(mockReq, mockRes, () => { });
    assert.strictEqual(mockRes.statusCode, 302);
...
test('POST /password/reset - should reject passwords shorter than 8 characters', async () => {
    ...
    const mockRes = createMockResponse();
    await getRouteHandler('/password/reset', 'post')?.(mockReq, mockRes, () => { });
    assert.strictEqual(mockRes.statusCode, 400);
