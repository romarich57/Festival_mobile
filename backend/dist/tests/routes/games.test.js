...
import { createTestFestival, createTestReservant, deleteTestGameFixtures, setupTests, teardownTests, } from '../test-helpers.js';
let testCounter = 0;
function nextPrefix() {
    testCounter += 1;
...
function getRouteStack(path, method) {
    const layer = gamesRouter.stack.find((item) => item.route?.path === path && item.route?.methods?.[method]);
...
async function createEditor(prefix) {
...
async function createMechanism(prefix, suffix) {
    const { rows } = await pool.query(`
...
async function cleanupGameFixtures(prefix) {
    await pool.query(`
...
test('GET / returns paginated games for benevole', async () => {
    ...
    assert.deepStrictEqual(firstPageRes.jsonData.items.map((item) => item.title), [`${prefix}_Alpha`, `${prefix}_Bravo`]);
    ...
    assert.deepStrictEqual(secondPageRes.jsonData.items.map((item) => item.title), [
        `${prefix}_Charlie`,
    ]);
...
test('GET /types returns distinct types including test fixtures', async () => {
    ...
    const matchingValues = res.jsonData.filter((value) => value.startsWith(prefix));
...
test('POST / allows organizer to create a game with mechanisms', async () => {
    ...
    assert.deepStrictEqual(res.jsonData.mechanisms.map((item) => item.id).sort((a, b) => a - b), [mechanismA.id, mechanismB.id]);
    await cleanupGameFixtures(prefix);
});
...
test('DELETE /:id returns conflict when game is allocated in a reservation', async () => {
    ...
    assert.strictEqual(res.statusCode, 409);
    assert.strictEqual(res.jsonData.error, 'Impossible de supprimer ce jeu car il est utilisé dans une réservation');
    await cleanupGameFixtures(prefix);
});
