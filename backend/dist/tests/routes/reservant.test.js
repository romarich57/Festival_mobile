...
import { createTestFestival, createTestReservant, generateTestEmail, setupTests, teardownTests } from '../test-helpers.js';
...
function getRouteStack(path, method) {
    const layer = reservantRouter.stack.find((item) => item.route?.path === path && item.route?.methods?.[method]);
...
async function createWorkflowForReservant(reservantId, festivalId, state = 'Pas_de_contact') {
    const { rows } = await pool.query(`
...
async function createContactEvent(contactId, workflowId, dateContact = new Date('2024-06-01T10:00:00Z')) {
    const { rows } = await pool.query(`
...
test('GET / - should return empty array when no data', async () => {
    ...
    assert.strictEqual(mockRes.statusCode, 200);
    assert.ok(Array.isArray(mockRes.jsonData));
    assert.ok(mockRes.jsonData.every((item) => !String(item.email ?? '').endsWith('@test.com')), 'Aucune fixture de test ne doit subsister après le nettoyage');
});
...
test('GET /:id/delete-summary - should include linked contacts workflows and reservations', async () => {
    ...
    assert.deepStrictEqual(res.jsonData.contacts.map((item) => item.id), [contact.id]);
    assert.deepStrictEqual(res.jsonData.workflows.map((item) => item.id), [workflow.id]);
    assert.deepStrictEqual(res.jsonData.reservations.map((item) => item.id), [reservation.id]);
});
