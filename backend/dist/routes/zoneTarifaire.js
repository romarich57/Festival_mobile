...
router.get('/:id/with-reservations', async (req, res) => {
    const { id } = req.params;
    try {
        const { rows } = await pool.query(`SELECT DISTINCT zt.id, zt.name, zt.festival_id, zt.nb_tables, zt.price_per_table, zt.nb_tables_available, zt.m2_price,
