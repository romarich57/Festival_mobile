...
function mapDatabaseError(err) {
    ...
    const withDetails = (error, details) => ({
        status: 400,
...
function parseSortOptions(query) {
    const sortByValue = typeof query.sortBy === 'string' ? query.sortBy.trim().toLowerCase() : '';
    const sortOrderValue = typeof query.sortOrder === 'string' ? query.sortOrder.trim().toLowerCase() : '';
    if (sortByValue) {
...
function buildPagedGameResponse(items, page, limit, total, sortBy, sortOrder) {
    return {
        items,
        pagination: {
            page,
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const listQuery = `${buildGameSelect(where)} ORDER BY ${sortOptions.orderBy} LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
        const [countResult, listResult] = await Promise.all([
            pool.query(countQuery, params),
            pool.query(listQuery, [...params, limit, offset]),
        ]);
        const total = countResult.rows[0]?.total ?? 0;
        res.json(buildPagedGameResponse(listResult.rows, page, limit, total, sortOptions.sortBy, sortOptions.sortOrder));
    }
    catch (err) {
    ...
    try {
      ...
      `, [
        ...
        if (typeof err.message === 'string' && err.message.startsWith('MISSING_MECHANISMS')) {
            ...
            return res.status(400).json(buildErrorBody('Mécanisme inexistant', `Mécanismes introuvables: ${missingIds}`));
        }
...
async function updateGame(req, res) {
    ...
    try {
       ...
       FROM games WHERE id = $1`, [gameId]);
        ...
        const mergedErrors = [];
        if (!merged.title)
            mergedErrors.push('title est requis');
        if (!merged.type)
            mergedErrors.push('type est requis');
        ...
        if (!merged.authors)
            mergedErrors.push('authors est requis');
