...
router.get('/stock/:festivalId', async (req, res) => {
    ...
    try {
        ...
        const festivalResult = await pool.query(`SELECT 
                stock_chaises as total_chaises
             FROM festival
             WHERE id = $1`, [festivalId]);
        ...
        const chaisesAlloueesResult = await pool.query(`SELECT COALESCE(SUM(rzp.nb_chaises), 0) as total_allouees
             FROM reservation_zone_plan rzp
             JOIN zone_plan zp ON rzp.zone_plan_id = zp.id
             WHERE zp.festival_id = $1`, [festivalId]);
...
router.get('/detail/:reservationId', async (req, res) => {
    ...
    try {
        ...
        const festivalResult = await pool.query(`SELECT 
             ...
             FROM festival
             WHERE id = $1`, [reservation.festival_id]);
...
router.post('/reservation', async (req, res) => {
    ...
    start_price, nb_prises, final_price, table_discount_offered = 0, direct_discount = 0, note, phone_number, address, siret, represented_editor_id, zones_tarifaires = [] // Nouvelle structure pour les zones tarifaires
     } = req.body;
    ...
    try {
        ...
        let editorId = null;
        let representedEditorId = null;
        let representedEditorEditorId = null;
        if (represented_editor_id !== undefined && represented_editor_id !== null && represented_editor_id !== '') {
            ...
            const representedEditorResult = await client.query(`SELECT id, editor_id
                 FROM reservant
                 WHERE id = $1 AND type = 'editeur'`, [representedEditorIdParsed]);
        ...
        if (zones_tarifaires && zones_tarifaires.length > 0) {
            for (const zone of zones_tarifaires) {
                ...
                await client.query(`INSERT INTO reservation_zones_tarifaires (reservation_id, zone_tarifaire_id, nb_tables_reservees, nb_chaises_reservees)
                     VALUES ($1, $2, $3, $4)`, [reservationId, zone.zone_tarifaire_id, zone.nb_tables_reservees, nbChaises]);
        ...
        if (totalChaisesReservees > 0) {
            const chaisesCheck = await client.query(`SELECT stock_chaises_available FROM festival WHERE id = $1 FOR UPDATE`, [festival_id]);
            ...
            await client.query(`UPDATE festival 
                 SET stock_chaises_available = stock_chaises_available - $1 
                 WHERE id = $2`, [totalChaisesReservees, festival_id]);
        }
        ...
        if (editorIdForGames) {
            ...
            const gamesResult = await client.query('SELECT id FROM games WHERE editor_id = $1', [editorIdForGames]);
...
router.put('/reservation/:id', async (req, res) => {
    ...
    try {
        ...
        const reservationInfo = await client.query(`SELECT festival_id FROM reservation WHERE id = $1`, [id]);
        ...
        const oldZones = await client.query(`SELECT zone_tarifaire_id, nb_tables_reservees, nb_chaises_reservees 
        ...
        if (oldTotalChaises > 0) {
            await client.query(`UPDATE festival 
                 SET stock_chaises_available = stock_chaises_available + $1 
                 WHERE id = $2`, [oldTotalChaises, festivalId]);
        }
        ...
        for (const zone of zones_tarifaires) {
            ...
            await client.query(`INSERT INTO reservation_zones_tarifaires (reservation_id, zone_tarifaire_id, nb_tables_reservees, nb_chaises_reservees)
                 VALUES ($1, $2, $3, $4)`, [id, zone.zone_tarifaire_id, zone.nb_tables_reservees, nbChaises]);
        ...
        if (newTotalChaises > 0) {
            const chaisesCheck = await client.query(`SELECT stock_chaises_available FROM festival WHERE id = $1 FOR UPDATE`, [festivalId]);
            const chaisesDisponibles = chaisesCheck.rows[0]?.stock_chaises_available || 0;
            ...
            await client.query(`UPDATE festival 
                 SET stock_chaises_available = stock_chaises_available - $1 
                 WHERE id = $2`, [newTotalChaises, festivalId]);
        }
        await client.query('COMMIT');
...
router.get('/reservation/:id/delete-summary', requireRole(RESERVATION_DELETE_ROLES), async (req, res) => {
    ...
    try {
        const { rowCount } = await pool.query('SELECT 1 FROM reservation WHERE id = $1', [reservationId]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Réservation introuvable' });
        }
        const zonesTarifairesResult = await pool.query(`SELECT rzt.zone_tarifaire_id,
                    zt.name AS zone_name,
                    rzt.nb_tables_reservees,
                    rzt.nb_chaises_reservees
             FROM reservation_zones_tarifaires rzt
             ...
             ORDER BY zt.name ASC`, [reservationId]);
        const zonePlanAllocationsResult = await pool.query(`SELECT rzp.zone_plan_id,
             ...
             ORDER BY zp.name ASC`, [reservationId]);
        const gamesAllocationsResult = await pool.query(`SELECT ja.id,
                    ja.game_id,
...
router.delete('/reservation/:id', requireRole(RESERVATION_DELETE_ROLES), async (req, res) => {
    ...
    try {
        ...
        const reservationInfo = await client.query(`SELECT festival_id FROM reservation WHERE id = $1`, [reservationId]);
        if (reservationInfo.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: 'Réservation introuvable' });
        }
        ...
        const zonesToRestore = await client.query(`SELECT zone_tarifaire_id, nb_tables_reservees, nb_chaises_reservees 
             FROM reservation_zones_tarifaires 
        ...
        if (totalChaisesToRestore > 0) {
            await client.query(`UPDATE festival 
                 SET stock_chaises_available = stock_chaises_available + $1 
                 WHERE id = $2`, [totalChaisesToRestore, festivalId]);
        }
        // 4. Supprimer les associations zones tarifaires
        await client.query(`DELETE FROM reservation_zones_tarifaires WHERE reservation_id = $1`, [reservationId]);
        // 5. Supprimer la réservation
        const deleteResult = await client.query(`DELETE FROM reservation WHERE id = $1 RETURNING *`, [reservationId]);
