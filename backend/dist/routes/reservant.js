...
function mapReservantDbError(err) {
    const code = err?.code;
...
router.get('/:id/delete-summary', async (req, res) => {
    ...
    try {
        ...
        const workflowsResult = await pool.query(`SELECT sw.id, sw.festival_id, sw.state, f.name AS festival_name
             FROM suivi_workflow sw
             LEFT JOIN festival f ON f.id = sw.festival_id
             WHERE sw.reservant_id = $1
             ORDER BY sw.id DESC`, [reservantId]);
        const reservationsResult = await pool.query(`SELECT r.id,
                    r.festival_id,
                    r.statut_paiement,
             ...
             ORDER BY r.id DESC`, [reservantId]);
        res.json({
...
router.post('/', async (req, res) => {
    ...
    try {
        const { rows: conflictRows } = await pool.query('SELECT name, email FROM reservant WHERE name = $1 OR email = $2', [name, email]);
