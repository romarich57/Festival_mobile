...
router.put('/reservation/:reservation_id/allocations/:zone_plan_id', async (req, res) => {
    ...
    try {
        const { rows: validationRows } = await pool.query(`SELECT r.festival_id AS reservation_festival, zp.festival_id AS zone_festival
             FROM reservation r
        ...
        const { rows: festivalRows } = await pool.query(`SELECT stock_chaises FROM festival WHERE id = $1`, [festivalId]);
        const totalChaises = Number(festivalRows[0]?.stock_chaises || 0);
        const { rows: alloueesRows } = await pool.query(`SELECT (
                COALESCE((
        ...
        const { rows: currentRows } = await pool.query(`SELECT nb_chaises
             FROM reservation_zone_plan
        ...
        const { rows } = await pool.query(`INSERT INTO reservation_zone_plan (reservation_id, zone_plan_id, nb_tables, nb_chaises)
             VALUES ($1, $2, $3, $4)
