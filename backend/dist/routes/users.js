...
async function isProtectedAdmin(userId) {
    const { rows } = await pool.query('SELECT id, login, email FROM users WHERE id = $1', [userId]);
    const user = rows[0];
    if (!user) {
        return false;
    }
    ...
    return (user.id === PROTECTED_ADMIN_ID ||
...
router.put('/:id', requireAdmin, async (req, res) => {
    ...
    const login = typeof req.body?.login === 'string' ? sanitize(req.body.login) : undefined;
    const roleInput = typeof req.body?.role === 'string' ? sanitize(req.body.role) : undefined;
    ...
    if (role !== undefined && role !== 'admin') {
      ...
      `, [id]);
            ...
            const isProtectedAdmin = target.id === PROTECTED_ADMIN_ID ||
