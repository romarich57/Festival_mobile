...
export function buildVerificationDeepLink(status) {
...
export function buildPasswordResetDeepLink(token) {
...
export function renderVerificationFallbackHtml({ status, deepLink, }) {
    const safeDeepLink = escapeHtml(deepLink);
    const contentByStatus = {
        success: {
            title: 'Email vérifié',
            message: 'Votre adresse email a bien été confirmée. Nous essayons d’ouvrir l’application mobile.',
        },
        expired: {
            title: 'Lien expiré',
            message: 'Le lien de vérification a expiré. Revenez dans l’application pour renvoyer un nouvel email.',
        },
        invalid: {
            title: 'Lien invalide',
            message: 'Ce lien de vérification est invalide. Revenez dans l’application pour demander un nouvel email.',
        },
        error: {
            title: 'Vérification indisponible',
            message: 'Une erreur est survenue pendant la vérification. Réessayez depuis l’application dans quelques instants.',
        },
    };
...
// Postconditions : Retourne une page HTML minimale qui tente d'ouvrir l'app.
export function renderPasswordResetFallbackHtml(token) {
...
function prefersHtmlResponse(req) {
    ...
    const acceptHeader = typeof req.get === 'function' ? req.get('accept') ?? '' : '';
...
function respondWithVerificationResult(req, res, status, httpStatus) {
...
router.get('/verify-email', async (req, res) => {
    ...
    `, [tokenHash]);
        ...
        if (!tokenMatch.email_verification_expires_at ||
...
router.post('/logout', async (req, res) => {
    ...
    if (refresh) {
        try {
            ...
            if (decoded?.jti) {
                await pool.query('UPDATE refresh_tokens SET revoked_at = NOW() WHERE jti_hash = $1', [hashRefreshTokenId(decoded.jti)]);
            }
        }
        catch (error) {
...
router.post('/refresh', async (req, res) => {
    ...
    `, [hashRefreshTokenId(decoded.jti)]);
        const stored = rows[0];
        if (!stored) {
            await pool.query('UPDATE refresh_tokens SET revoked_at = NOW() WHERE user_id = $1 AND revoked_at IS NULL', [decoded.id]);
            return res.status(403).json({ error: 'Refresh token invalide ou expiré' });
        }
