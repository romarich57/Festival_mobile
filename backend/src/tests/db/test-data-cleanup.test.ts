...
test('cleanupTestData supprime les fixtures games_test du catalogue jeux', async () => {
  ...
  const editorResult = await pool.query<{ id: number }>(
    `
  ...
  const mechanismResult = await pool.query<{ id: number }>(
    `
  ...
  const gameResult = await pool.query<{ id: number }>(
    `
  ...
  await pool.query(
    'INSERT INTO game_mechanism (game_id, mechanism_id) VALUES ($1, $2)',
    [gameId, mechanismId],
  )

  await cleanupTestData()

  const remainingTypes = await pool.query<{ type: string }>(
    "SELECT type FROM games WHERE type LIKE 'games_test_%'",
  )
  const remainingMechanisms = await pool.query<{ name: string }>(
    "SELECT name FROM mechanism WHERE name LIKE 'games_test_%'",
  )
  const remainingEditors = await pool.query<{ email: string }>(
    "SELECT email FROM editor WHERE email LIKE 'games_test_%@editor.test.com'",
  )
